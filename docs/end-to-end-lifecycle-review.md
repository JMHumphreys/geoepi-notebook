...
- Rendered-site checks: the home page, Start Here, Research Practices, all eight lifecycle landing pages, Resources, Templates, representative subpages, raw-template links, Mermaid diagrams, tables, sidebars, search-index output, and static resource anchors were inspected in generated HTML. Long status labels, tables, sidebar content, and raw-template links were checked structurally for narrow-width risks. Interactive local-file browser inspection was unavailable in this environment; the static-anchor requirement reduces reliance on browser execution but is not a claim of full accessibility conformance or a substitute for human visual review.
...
|---|---|---|---|---|---|---|---|
| E2E-001 | `resources/index.qmd` | resource-page defect | The initial listing requested `author` and `categories`, while the catalog defines `author_or_organization` and `topics`; the URL was not displayed. | Scientists could not reliably see who issued a resource or open its source landing page. | Use a Quarto custom listing template so titles render as static descriptive hyperlinks using the catalog `url`; no duplicate URL field was added. | `resources/index.qmd`, `resources/resource-table.ejs.md` | corrected |
| E2E-002 | `scripts/validate_site.py` and rendered Resources page | resource-page defect | Existing validation did not require actual rendered anchors for catalog URLs. | A catalog could appear complete while source links were absent or existed only as browser-side behavior. | Compare each catalog URL with a static rendered anchor, require descriptive title text, and reject empty anchors; no browser-side JavaScript is required. | `scripts/validate_site.py` | corrected |
...
| E2E-009 | `resources/catalog.yml` | resource-page defect | Several owner-supplied seed-resource URLs had been replaced during catalog development by publisher pages, search pages, or alternate sites. | Lab members could be sent to a general publisher or search page instead of the specific teaching or reference resource selected for the Lab Book. | Restored all 16 original owner-supplied URLs and added regression validation preventing silent substitution. Owner-selected URLs and external-authority metadata serve different purposes; future substitutions must be explicit. | `resources/catalog.yml`, `scripts/validate_site.py` | corrected |
